...
JSON_CONFIG_LOCATION = "/usr/local/bin/datacollector/myconfig.json"
...
class DataCollectorConsumer(object):
    ...
    ROUTING_KEY = None
    MAX_HTTP_RETRIES = 5
    LWM2M_OBJECTS = [3301,3303,3304,3324,3325,3330,3348]

    def __init__(self, amqp_url, lwm2m_url):
        ...
        self._url = amqp_url
        self._lwm2murl = lwm2m_url
        self._lwm2m_clients = set()
    ...
    def on_message(self, unused_channel, basic_deliver, properties, body):
        ...
        try:
            ...
            #get client name
            lwm2m_client = str(json_parsed['endpoint'])
            #set of clients registered (TODO::maintain registration observer)
            self._lwm2m_clients.add(lwm2m_client)

            ret = self.lwm2m_sync_time(lwm2m_client)
            print str(json.loads(ret.content)['status']) + ", synchronized client: " + str(lwm2m_client)
            for lwm2m_object in self.LWM2M_OBJECTS:
...
__author__ = 'Riccardo Pozza, r.pozza@surrey.ac.uk'
    ...
    def lwm2m_start_observation(self, client, object):
        fullurl = self._lwm2murl + client + "/" + str(object) + "/" + str(0) + "/observe"
        tries = 0
        while tries < self.MAX_HTTP_RETRIES:
            time_now = datetime.datetime.now().strftime("%Y-%m-%dT%H:%M:%SZ")
            r = requests.post(fullurl)
            tries += 1
            if r.status_code == requests.codes.ok:
                if json.loads(r.content)['status'] == 'CONTENT':
                    print "Started Observation at: " + str(time_now) + " of " + client + "/" + str(object) + "/" + str(0)
                    return r
            time.sleep(tries * 0.2)

        print "Maximum Number of Attempts achieved!"
        return r

    def lwm2m_sync_time(self, client):
        header_json = {'Content-Type': 'application/json'}
        parameters = {'format': 'Text'}
        fullurl = self._lwm2murl + client + "/3/0/13" # time
        tries = 0
        while tries < self.MAX_HTTP_RETRIES:
            time_now = datetime.datetime.now().strftime("%Y-%m-%dT%H:%M:%SZ")
...
def main():
    ...
    # Setting LWM2M SERVER options
    LWM2M_SERVER = str(config_data["LWM2MServer"])
    LWM2M_PORT = str(config_data["LWM2MServerPort"])
    LWM2M_URL = 'http://' + LWM2M_SERVER + ':' + LWM2M_PORT + '/api/clients/'
    consumer = DataCollectorConsumer(AMQP_URL, LWM2M_URL)
    try:
        consumer.run()
